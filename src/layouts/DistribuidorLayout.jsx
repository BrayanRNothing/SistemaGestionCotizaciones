...
const DistribuidorLayout = () => {
  ...
  const handleTabChange = (tab) => {
    ...
    window.dispatchEvent(new CustomEvent('changeDistribuidorTab', { detail: tab }));
  };

  const menuItems = [
    { name: 'Inicio', icon: '🏠', action: () => handleTabChange('home') },
    { name: 'Solicitar', icon: '➕', action: () => handleTabChange('solicitar') },
    { name: 'Ajustes', icon: '⚙️', action: () => handleTabChange('ajustes') },
  ];
      ...
      <header ref={vantaRef} className="shadow-lg px-6 py-4 flex justify-between items-center z-10 relative overflow-hidden min-h-[80px] bg-gradient-to-r from-blue-900 to-blue-700">
        ...
        <div className="relative z-10 flex justify-between items-center w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white drop-shadow-[0_2px_8px_rgba(37,99,235,0.5)]">🏪 Portal Distribuidor</h2>
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-blue-900 border-t border-blue-800 shadow-lg pb-safe z-50">
        <div className="max-w-2xl mx-auto px-6 py-2">
          <div className="flex items-center justify-around relative h-16">
            ...
            {/* Botón central de Solicitar (flotante) */}
            <button
